...
declare module "when" {
   ...
   */
  export function join<T>(...promises: Promise<T>[]): Promise<T[]>;
  /**
   * Joins multiple promises into a single returned promise.
   * @return a promise that will fulfill when *all* the input promises
   * have fulfilled, or will reject when *any one* of the input promises rejects.
   */
  export function join<T>(...promises: any[]): Promise<T[]>;
   ...
   */
  export function resolve<T>(promise: Promise<T>): Promise<T>;
  export function resolve<T>(value?: T): Promise<T>;

  export function reject<T>(value: any): Promise<T>;
